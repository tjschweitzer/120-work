...
// this makes the 2d array of cols by rows
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    for (var j =0; j<rows; j++){
      arr[i][j][0]=random(255);
      arr[i][j][1]=random(255);
      arr[i][j][2]=random(255);
    }
  }
  ...
  for (var i = 1; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      // moves the color or each circle one to the left
        balls[i-1][j][0].change(balls[i][j][0].r(), balls[i][j][0].g(), balls[i][j][0].b());
    }
    // random color to the right most row
  balls[rows-1][i][0].change(random(255),random(255),random(255));
  }
}
